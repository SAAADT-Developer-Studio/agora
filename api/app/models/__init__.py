...
class Article(Base):
    ...
    news_provider_key: Mapped[str] = mapped_column(
        String, ForeignKey("news_provider.key")
    )
    news_provider: Mapped["NewsProvider"] = relationship(
        "NewsProvider", back_populates="articles"
    )
    ...
    cluster: Mapped[Optional["Cluster"]] = relationship(
        "Cluster", back_populates="articles"
    )
...
class NewsProvider(Base):
    ...
    articles: Mapped[List["Article"]] = relationship(
        "Article", back_populates="news_provider"
    )
